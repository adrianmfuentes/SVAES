...
class ConfluenceConnector(BaseHttpConnector):
    ...
    def _get_site_url(self, config: Dict[str, Any]) -> str:
        url = self._get_base_url(config).rstrip("/")
        if url.endswith("/wiki"):
            url = url[:-5]
        return url

    def _get_health_url(self, config: Dict[str, Any]) -> str:
        return f"{self._get_site_url(config)}/wiki/rest/api/user/current"

    def _get_fetch_url(self, ref: str, config: Dict[str, Any]) -> str:
        return f"{self._get_site_url(config)}/wiki/rest/api/content/{ref}"

    def _get_fetch_params(self, config: Dict[str, Any]) -> Dict[str, Any] | None:
        return {"expand": "version"}

    def _get_list_url(self, filter_params: Dict[str, Any], config: Dict[str, Any]) -> str:
        return f"{self._get_site_url(config)}/wiki/rest/api/content/search"
