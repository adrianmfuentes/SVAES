...
def upgrade() -> None:
    ...
    op.execute("CREATE TYPE severity_type_new AS ENUM ('INFO', 'LOW', 'MEDIUM', 'HIGH', 'CRITICAL')")
    op.add_column('verification_rule', sa.Column('_severity_old', sa.Enum('OBLIGATORIA', 'RECOMENDADA', 'INFORMATIVA', name='severity_type', create_type=False), nullable=True))
    ...
    op.add_column('verification_rule', sa.Column('severity', sa.Enum('INFO', 'LOW', 'MEDIUM', 'HIGH', 'CRITICAL', name='severity_type_new', create_type=False), nullable=False, server_default='HIGH'))
    op.execute("UPDATE verification_rule SET severity = CASE _severity_old::text WHEN 'OBLIGATORIA' THEN 'CRITICAL'::severity_type_new WHEN 'RECOMENDADA' THEN 'MEDIUM'::severity_type_new WHEN 'INFORMATIVA' THEN 'INFO'::severity_type_new ELSE 'HIGH'::severity_type_new END")
