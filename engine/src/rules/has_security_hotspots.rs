...
pub fn evaluate(artifacts: &[Artifact], rule_config: &VerificationRule) -> RuleEvaluation {
    ...
    // Un campo ausente o con un tipo no numérico no es "cero hotspots": es un
    // dato que el conector no reportó o reportó mal - se trata como violación
    // (falla cerrado) en lugar de excluir el artefacto en silencio.
    let violations: Vec<String> = matching.iter()
        .filter_map(|a| match a.metadata.get(field).and_then(|v| v.as_f64()) {
            Some(val) if val > threshold => Some(format!("'{}': {} hotspots de seguridad", a.id, val as u64)),
            Some(_) => None,
    ...
    if violations.is_empty() {
        ...
        RuleEvaluation {
            ...
            message: Some("rule_evidence.error.has_security_hotspots".to_string()),
            message_params: Some(json!({
